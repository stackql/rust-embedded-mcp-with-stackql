...
```sh
# No build.rs here: point STACKQL_MCP_BUNDLE_FILE at a .mcpb you already have. Building
# minimal-vendored once fetches the pinned bundle and prints its cached path as a cargo warning.
cargo build -p minimal-vendored
STACKQL_MCP_BUNDLE_FILE=/path/to/stackql-mcp-<platform>.mcpb cargo build -p stackql-agent --features vendored --release
```
